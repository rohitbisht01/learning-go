...
import (
	...
	"github.com/rohitbisht01/url-shortener/api/utils"
)
...
func ShortenUrl(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&body); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Connot Parse JSON"})
		return
	}
	...
	// storing custom short url
	r := database.CreateClient(0)
	defer r.Close()

	val, _ = r.Get(database.Ctx, id).Result()
	if val != "" {
		c.JSON(http.StatusForbidden, gin.H{"error": "URL CustomShort already exists"})
		return
	}
	...
	// pong, err := r.Ping(database.Ctx).Result()
	// fmt.Println("Redis connection:", pong, err)

	err = r.Set(database.Ctx, id, body.URL, body.Expiry*3600*time.Second).Err()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Unable to connect to redis server"})
		return
	}
